...
import { UserInvestmentHistory, formatCurrency } from '../../Api/ApiActions';
...
export default function PlanHistory() {
    ...
    const getHistory = async () => {
        setLoading(true);
        const response = await UserInvestmentHistory(user.userid);
        setLoading(false);
                                    ...
                                    >
                                        <View className="flex-row items-center justify-between space-x-2">
                                            <View className='flex-1 items-start'>
                                                <Text className="font-bold text-lg " > {item.value.name} PLAN</Text>
                                                <Text className="font-semibold text-md text-gray-600" > {user.symbol + formatCurrency(item.value.amount)}</Text>
                                            </View>

                                            <View className='flex-1 items-end '>
                                                <Text className="font-semibold text-lg " > {item.value.duration}</Text>
